...
import com.greencarwash.notification.exception.ResourceNotFoundException;
...
public class NotificationRecordService {
    ...
    public NotificationRecordService(NotificationRecordRepository repository) { this.repository = repository; }
    ...
    public RecordResponse update(Long id, RecordRequest request) {
        NotificationRecord entity = repository.findById(id).orElseThrow(() -> new ResourceNotFoundException("Record not found: " + id));
    ...
    private RecordResponse toResponse(NotificationRecord e) { return new RecordResponse(e.getId(), e.getReferenceId(), e.getStatus(), e.getDescription(), e.getCreatedAt()); }
}
